...
class TestSaleOrder(TestSale):
    ...
    def test_so_confirm_ok(self):
        so = self._create_so()
        amount = 100
        comment = 'test_so_confirm_ok'
        so.partner_id._credit_point_update(amount, comment=comment)
        so.partner_id.update_history(amount, 'increase', comment=comment)
        self.assertEqual(so.state, 'draft')
        so.action_confirm()
        self.assertNotEqual(so.state, 'draft')
        self.assertEqual(so.partner_id.credit_point, 90)
        self.assertEqual(so.partner_id.yearly_point_increase, 100)
